...
func TestSummarizeInvoice(t *testing.T) {
	entry := func(userID int, user, task string, taskID int, hours float64) harvest.TimeEntry {
	...
	t.Run("given entries from several people when summarized then groups by person and task sorted by name", func(t *testing.T) {
		entries := []harvest.TimeEntry{
			entry(2, "Sam Chen", "Design", 201, 2.0),
			entry(1, "Alex Rivera", "Development", 202, 1.5),
			entry(1, "Alex Rivera", "Design", 201, 0.5),
			entry(1, "Alex Rivera", "Development", 202, 2.0),
		}
